...
class FantasyCardFactory(CardFactory):
    ...
    def create_creature(self, name_or_power) -> Card:
        creature_stats = {
            "Dragon": {"cost": 5, "rarity": Rarity.level_3.value, "attack": 7, "health": 5},
            "Goblin": {"cost": 2, "rarity": Rarity.level_1.value, "attack": 5, "health": 2}
        }
        if isinstance(name_or_power, str):
            stats = creature_stats[name_or_power]
            return (CreatureCard(name_or_power, stats['cost'], stats['rarity'], stats['attack'], stats['health']))

        if isinstance(name_or_power, int):
            if name_or_power >= 7: name = "Dragon"
            else: name = "Goblin"
            stats = creature_stats[name]
            return (CreatureCard(name, stats['cost'], stats['rarity'], stats['attack'], stats['health']))
